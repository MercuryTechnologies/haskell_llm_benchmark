...
calculateScore rolls = calculateScore' (toFrames rolls)
  where
    calculateScore' :: Either BowlingError [[Int]] -> Int
    calculateScore' (Left _) = 0  -- Should not happen, validation already done
    calculateScore' (Right frames) = sum (map calculateFrameScore frames)
...
toFrames rolls = toFrames' rolls [] 1
  where
    ...
    toFrames' [] acc frameNum
      | length acc == 10 = Right acc
    ...
    toFrames' (roll:rest) acc frameNum
      ...
      | otherwise =
          case rest of
            (roll2:rest2) | (roll + roll2) == 10 && frameNum < 10 -> toFrames' rest2 (acc ++ [[roll, roll2]]) (frameNum + 1)
                           | (roll + roll2) < 10 && frameNum < 10 -> toFrames' rest2 (acc ++ [[roll, roll2]]) (frameNum + 1)
                           | otherwise -> Left IncompleteGame
