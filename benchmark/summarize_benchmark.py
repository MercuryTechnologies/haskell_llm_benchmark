...
def extract_key_metrics(dirname: str) -> Dict[str, Any]:
    ...
    if metrics["completed_tests"] > 0:
        ...
        metrics["percent_cases_well_formed"] = round(
            100 * (1 - with_malformed_responses / metrics["completed_tests"]), 1
        )
    ...
    if metrics.get("model"):
        ...
        # Append reasoning_effort or thinking_tokens to differentiate runs
        reasoning_effort = None
        thinking_tokens = None
        for results in all_results:
            if results.get("reasoning_effort"):
                reasoning_effort = results["reasoning_effort"]
            if results.get("thinking_tokens"):
                thinking_tokens = results["thinking_tokens"]
        if reasoning_effort:
            model_name = f"{model_name} (effort={reasoning_effort})"
        if thinking_tokens:
            model_name = f"{model_name} (thinking={thinking_tokens})"
...
def plot_comparison(df: pd.DataFrame, output_path: Optional[str] = None) -> None:
    ...
    # Add title
    plt.suptitle("Haskell LLM Benchmark", fontsize=26, fontweight="bold", y=0.98)

    # Add subtitle
    plt.title("Model Performance: Pass Rate vs. Cost", fontsize=18, pad=20, color="#444444")
    ...
    plt.figtext(
        0.5,
        0.01,
        f"Generated on {today} | pass rate based on {pass_rate_column}",
        ha="center",
        fontsize=12,
        fontweight="bold",
        color="#666666",
    ...
    dummy_seconds_line = ax1.plot(
        [], [], color=seconds_color, linestyle="None", marker="None", label="time per test"
    )[0]
    ...
    filtered_labels1 = [lab for lab in labels1 if lab != dummy_label]
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        help=(
            "Benchmark directory names (if not provided, uses all dirs in tmp.benchmarks except"
            " polyglot-benchmark)"
        ),
    )
    parser.add_argument(
        "--output", "-o", help="Output path prefix (default: generates a timestamp directory)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Minimum completed tests to include a run (e.g. 112)",
    )
    parser.add_argument(
        "--include-cached",
        action="store_true",
        help="Include cached results from benchmark/cached_results.json",
    )
    ...
    if not args.dirnames:
        if BENCHMARK_DNAME.exists() and BENCHMARK_DNAME.is_dir():
            dirnames = [
                d
                for d in BENCHMARK_DNAME.iterdir()
    ...
    for dirname in dirnames:
        ...
        if metrics:
            if args.min_tests and metrics.get("completed_tests", 0) < args.min_tests:
                print(
                    f"  Skipping {dirname} ({metrics['completed_tests']} tests < {args.min_tests})"
                )
                continue
    ...
    if args.include_cached:
        cached_path = Path(__file__).parent / "cached_results.json"
        if cached_path.exists():
            ...
            with open(cached_path) as f:
                cached = json.load(f)
            for entry in cached:
                if args.min_tests and entry.get("completed_tests", 0) < args.min_tests:
                    continue
                metrics_list.append(entry)
            print(f"Loaded {len(cached)} cached results from {cached_path}")
        else:
            print(f"Warning: cached results file not found at {cached_path}")
