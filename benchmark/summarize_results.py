...
def plot_benchmark_comparison(df: pd.DataFrame, output_path: str = None) -> None:
    ...
    plt.figtext(
        0.5,
        0.01,
        f"Generated on {today} | Haskell LLM Benchmark",
        ha="center",
        fontsize=12,
        fontweight="bold",
        color="#666666",
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        help=(
            "Benchmark directory paths to analyze (if none provided, uses all dirs in"
            " tmp.benchmarks except polyglot-benchmark)"
        ),
    )
    ...
    if not args.dirnames:
        ...
        if tmp_benchmarks.exists() and tmp_benchmarks.is_dir():
            dirnames = [
                d for d in tmp_benchmarks.iterdir() if d.is_dir() and d.name != "polyglot-benchmark"
            ]
        else:
    ...
    if df is not None:
        ...
        for model in duplicate_models:
            ...
            for idx, row in df.loc[mask].iterrows():
                ...
                # Look for .aider.chat.history.md to determine thinking mode.
                # From 2025-04-02: model does NOT have thinking enabled
                # (whole edit format, prompt cache, infinite output, NO think tokens)

                # Special case: only the specific directory for the thinking
                # experiment has thinking enabled (2025-04-03-10-39-27)
                if "2025-04-03-10-39-27" in str(dir_path) or "2025-04-03-10-39-27" in str(
                    row.get("dir_name", "")
                ):
                    is_thinking = True
                ...
                if is_thinking and model == "claude-3-7-sonnet-20250219":
                    ...
                    suffix = idx.split("--")[-1].split("-full")[0] if "--" in str(idx) else idx
                    df.loc[idx, "Model"] = f"{model} ({suffix})"
